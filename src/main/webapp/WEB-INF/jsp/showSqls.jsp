...
$("body").click(function(i){ !$(i.target).parents(".select").first().is(s) ? _hide():"";});})})
			...
			<div id="MainForm">
				<div class="form_boxA">
						...
						<tr>
							<c:forEach var="sql" items="${ListSqls}" begin="0"
								...
								<tr>
									...
									<td>${sql.id}</td>
									<td><textarea id='${sql.id}'
											style="width: 400px; height: 120px;">${sql.exe_sql}</textarea></td>
									<td style="width: 200px; height: 120px;"><textarea id='instruction${sql.id}'
											style="width: 200px; height: 120px;">${sql.instruction}</textarea></td>
									<td><input id='start${sql.id}' type="text"
										value='startTime(yyyy-MM-dd)'></td>
									<td><input id='interval${sql.id}' type="text"
										value='interval(整数)'></td>
									<td id='ru${sql.id}'>${sql.is_running}</td>
									<td><ul id='exe'>
											<li id='li${sql.id}' onclick='update(${sql.id})'>更改</li>|
											<li id='exe${sql.id}' onclick='exe(${sql.id})'>重新执行</li>
										</ul></td>
								</tr>
							</c:forEach>
						</tr>
					...
					<p class="msg">共找到${count}条sql记录</p>
				</div>
			</div>
			<!-- /MainForm -->
			<!-- PageNum -->
			<ul id="PageNum">
				...
				<c:forEach begin="1" end="${count/10+1}" step="1" varStatus="i">
					...
					<c:if test="${start!=i.index}">
						<li><a href="showSqls.do?start=${i.index}">${i.index}</a></li>
					</c:if>

				</c:forEach>
				<li><a href="showSqls.do?start=${start+1}">下一页</a></li>
